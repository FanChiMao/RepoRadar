...
- App 內 daily sync、weekly report 與 AI 排程。
...
| F-10 | AI 排程：依 repo、時間範圍、模型與 Teams Webhook 定期整理 Issue 變動 | P1 |

## 模型選擇

- Chat/RAG：`gemini-3.5-flash`、`gemini-2.5-pro`、`gemma-4-26b-a4b-it`，並可使用 `.env` 設定的 Azure/OpenAI/Anthropic 模型。
- Arrange：`gemini-2.5-pro`、`gemini-3.5-flash`，並可使用 `.env` 設定的 Azure/OpenAI/Anthropic 模型。
- AI 排程：任務可選 preferred model；後端依任務模型、Azure 模型、Gemini fallback 的順序嘗試，結果會標示實際使用模型。
